...
function saveObserved(): void {
  try {
    ...
    console.warn('Failed to save observed limits:', e);
  }
}
...
function loadObserved(): void {
  try {
    ...
    console.warn('Failed to load observed limits:', e);
  }
}
...
function extractOpenAIResult(data: OpenAIResponse): { text: string; usage: AskResult['usage'] } {
  return {
...
function buildMessages(userId: string, question: string): ChatMessage[] {
  ...
  for (const h of getHistory(userId)) {
    messages.push({
      role: h.role,
      content: h.content.length > HISTORY_MESSAGE_LIMIT ? `${h.content.slice(0, HISTORY_MESSAGE_LIMIT - 1)}…` : h.content,
    });
  }
...
export async function ask(question: string, overrideModel: string | null, userId: string): Promise<AskResult> {
  ...
  if (m.provider === 'openrouter') {
    ...
    const r = extractOpenAIResult(data);
...
export async function refreshGroqLimits(model: string): Promise<RateLimits | null> {
  ...
  if (cached && Date.now() - cached.at < OBSERVED_LIMITS_TTL_MS) return cached.rateLimits;
  try {
